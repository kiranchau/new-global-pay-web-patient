...
import { allNotification, removeNotification } from '../../modules/Auth/_redux/authCrud';
...
class Notifications extends Component {
    ...
    //Nofitifactions Api calling.
    showNotification = () => {
        this.setState({ loader: true, spinner: true, animate: true }, () => { })
        allNotification(localStorage.getItem('StudiesId'), localStorage.getItem('id'))
            .then((res) => {
                this.state.notifications = res.data.notification
                this.setState({ loader: false, spinner: false, animate: false }, () => { })
                const nDiv = document.getElementById('right-left' + res.data.notification[0].id);
                nDiv.classList.remove('animate__animated');
                nDiv.classList.remove('animate__fadeInLeft');
            }).catch((err) => {
    ...
    //Remove notification api calling.
    removeNotificationRecord = (id) => {
        ...
        removeNotification(id).then((res) => {
            if (res.data.status === 0) {
                if (!toast.isActive(this.toastId.current)) {
                    this.toastId.current = toast.success(this.props.t('Notification archived successfully'), {
                        position: "top-center",
                        autoClose: 3000,
                                ...
                                <div>
                                    <div className='table-wrapper'>
                                        <div className='table-header'>
                                            <h6 className='visible text-white'>{t("All notifications")}</h6>
                                            <div className="input-group rounded">
                                                <button className='btn btn-primary show-archive-btn' onClick={() => this.props.history.push("/showArchieved")}>
                                                    <img src="/media/images/archive-white.svg" alt=""/>
                                                    {t("Show Archived")}
                                                </button>
                                            </div>
                                        </div>
                                                                        ...
                                                                        <div className='right-content'>
                                                                            <div className='study_name'>
                                                                                ...
                                                                                <div className='tooltip-show'>
                                                                                    <svg onClick={() => this.removeNotificationRecord(Obj.id)} xmlns="http://www.w3.org/2000/svg" width="20" height="19" viewBox="0 0 20 19">
                                                                                    ...
                                                                                    <span className='custom-tooltip'>{t("Archive")}</span>
                                                                                </div>
                                                                            </div>
                                                                            <p>
                                                                ...
                                                                <div colSpan={5}>
                                                                    <div className="data-not-found">
                                                                        <img
                                                                            ...
                                                                            height="100"
                                                                        />
                                                                        <label className="text-center no-data-text">{t("No notifications available")}</label>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                }
